...
interface OralExaminationFindings {
  ...
  additionalTests: {
    ...
    mobilityTests: { tooth: string; degree: '0' | '1' | '2' | '3'; notes: string }[]
    percussionTests: { tooth: string; result: 'negative' | 'positive' | 'tender'; notes: string }[]
...
export function DentalExaminationForm({
  ...
  const loadExistingInvestigations = async () => {
    if (!consultationId || investigationsLoaded) return
    ...
    try {
      ...
      const { data: investigations, error } = await supabase
        ...
        .eq('category', 'diagnostic_imaging')
      ...
      if (investigations && investigations.length > 0) {
        const imagingTypes = investigations.map(inv => inv.investigation_name)
        ...
        setFormData(prev => ({
          ...
          examinationFindings: {
            ...
            diagnosticImaging: {
              required: true,
              types: imagingTypes,
              findings: latestInvestigation.results_summary || '',
              notes: latestInvestigation.clinical_indication || '',
              status: latestInvestigation.status || 'ordered',
              followUpRequired: latestInvestigation.follow_up_required || false
            }
          }
        }))

        console.log('📋 Loaded existing investigations:', investigations)
      }
  ...
  React.useEffect(() => {
    if (consultationId && !investigationsLoaded) {
      loadExistingInvestigations()
    }
  }, [consultationId, investigationsLoaded])
  ...
  const updateToothCondition = (tooth: ToothData, condition: ToothCondition, notes?: string) => {
    ...
    setFormData(prev => {
      const existingIndex = prev.examinationFindings.dentalFindings.findIndex(t =>
        t.quadrant === tooth.quadrant && t.position === tooth.position
      )
      ...
      if (existingIndex >= 0) {
        ...
        // Add new tooth
        newDentalFindings = [...prev.examinationFindings.dentalFindings, updatedTooth]
      }
  ...
  // Add test result
  const addTestResult = (testType: keyof typeof formData.examinationFindings.additionalTests, testData: any) => {
    setFormData(prev => ({
      ...prev,
      examinationFindings: {
        ...prev.examinationFindings,
        additionalTests: {
          ...
          [testType]: [...prev.examinationFindings.additionalTests[testType], testData]
        }
      }
    }))
  }
      ...
      examinationFindings: {
        ...
        diagnosticImaging: {
          ...
          types: prev.examinationFindings.diagnosticImaging.types.includes(type)
            ? prev.examinationFindings.diagnosticImaging.types.filter(t => t !== type)
  ...
  const saveInvestigations = async () => {
    if (!formData.examinationFindings.diagnosticImaging.required || !consultationId) {
      return
    }
    ...
    try {
      ...
      // Save each imaging type as a separate investigation
      const investigations = formData.examinationFindings.diagnosticImaging.types.map(type => ({
        consultation_id: consultationId,
        investigation_type: 'imaging',
        investigation_name: type,
        category: 'diagnostic_imaging',
        clinical_indication: formData.examinationFindings.diagnosticImaging.notes,
        status: formData.examinationFindings.diagnosticImaging.status || 'ordered',
        results_summary: formData.examinationFindings.diagnosticImaging.findings,
        follow_up_required: formData.examinationFindings.diagnosticImaging.followUpRequired || false,
        urgency: 'routine'
      }))
      ...
      if (investigations.length > 0) {
        const { error } = await supabase
          ...
          .upsert(investigations, {
            onConflict: 'consultation_id,investigation_name',
        ...
        if (error) {
          ...
          console.log('✅ Investigations saved successfully')
        }
      }
    } catch (error) {
      console.error('Error saving investigations:', error)
          ...
          <Tabs defaultValue="extra-oral" className="space-y-4">
            ...
            <TabsContent value="extra-oral" className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Textarea
                    id="face-symmetry"
                    value={formData.examinationFindings.extraOralExamination.faceSymmetry}
                ...
                <div>
                  ...
                  <Textarea
                    id="lymph-nodes"
                    value={formData.examinationFindings.extraOralExamination.lymphNodes}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
                ...
                <div>
                  ...
                  <Textarea
                    id="tmj"
                    value={formData.examinationFindings.extraOralExamination.tmjAssessment}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
                ...
                <div>
                  ...
                  <Textarea
                    id="muscle-function"
                    value={formData.examinationFindings.extraOralExamination.muscleFunction}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
              ...
              <div>
                ...
                <Textarea
                  id="skin-lesions"
                  value={formData.examinationFindings.extraOralExamination.skinLesions}
                  onChange={(e) => setFormData(prev => ({
                    ...prev,
                    examinationFindings: {
                      ...prev.examinationFindings,
              ...
              <div>
                <Label htmlFor="extra-oral-notes">Additional Extra-oral Notes</Label>
                <Textarea
                  id="extra-oral-notes"
                  value={formData.examinationFindings.extraOralExamination.notes}
                  onChange={(e) => setFormData(prev => ({
                    ...prev,
                    examinationFindings: {
                      ...prev.examinationFindings,
            ...
            <TabsContent value="intra-oral" className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    value={formData.examinationFindings.intraOralExamination.oralHygiene}
                    onValueChange={(value: 'excellent' | 'good' | 'fair' | 'poor') => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
                        intraOralExamination: {
                ...
                <div>
                  ...
                  <Textarea
                    id="gingiva"
                    value={formData.examinationFindings.intraOralExamination.gingivaCondition}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
                ...
                <div>
                  ...
                  <Textarea
                    id="periodontal"
                    value={formData.examinationFindings.intraOralExamination.periodontalStatus}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
                ...
                <div>
                  ...
                  <Textarea
                    id="oral-mucosa"
                    value={formData.examinationFindings.intraOralExamination.oralMucosa}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
                ...
                <div>
                  ...
                  <Textarea
                    id="tongue"
                    value={formData.examinationFindings.intraOralExamination.tongue}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
                ...
                <div>
                  ...
                  <Textarea
                    id="palate"
                    value={formData.examinationFindings.intraOralExamination.palate}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      examinationFindings: {
                        ...prev.examinationFindings,
              ...
              <div>
                <Label htmlFor="intra-oral-notes">Additional Intra-oral Notes</Label>
                <Textarea
                  id="intra-oral-notes"
                  value={formData.examinationFindings.intraOralExamination.notes}
                  onChange={(e) => setFormData(prev => ({
                    ...prev,
                    examinationFindings: {
                      ...prev.examinationFindings,
            ...
            <TabsContent value="dental-chart" className="space-y-4">
              <div>
                ...
                <DentalChart
                  key={`dental-chart-${formData.examinationFindings.dentalFindings.length}-${JSON.stringify(formData.examinationFindings.dentalFindings).slice(0, 50)}`}
                ...
                <Card className="border-2 border-blue-200 mt-4">
                  <CardHeader>
                    <CardTitle className="text-lg">
                      Edit Tooth {selectedTooth.quadrant}{selectedTooth.position}
                      {selectedTooth.number && ` (Universal #${selectedTooth.number})`}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value: ToothCondition) =>
                          updateToothCondition(selectedTooth, value, selectedTooth.notes)
                        }
                        disabled={isReadOnly}
                      >
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        onChange={(e) => setSelectedTooth({...selectedTooth, notes: e.target.value})}
            ...
            <TabsContent value="tests" className="space-y-4">
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    Order investigations and update results in follow-up appointments
                  </p>
                </div>
                ...
                <div>
                  <div className="flex items-center space-x-2 mb-4">
                    <Checkbox
                      id="imaging-required"
                      checked={formData.examinationFindings.diagnosticImaging.required}
                      ...
                      disabled={isReadOnly}
                    />
                    <Label htmlFor="imaging-required" className="font-medium text-base">
                  ...
                  {formData.examinationFindings.diagnosticImaging.required && (
                    <Card className="border-l-4 border-l-blue-500">
                      <CardContent className="p-4 space-y-4">
                        <div>
                          <Label className="text-sm font-medium">Imaging Types (select all that apply)</Label>
                          <div className="grid grid-cols-2 gap-2 mt-2">
                            {IMAGING_TYPES.map((type) => (
                              <div key={type} className="flex items-center space-x-2">
                                <Checkbox
                                  id={type}
                                  checked={formData.examinationFindings.diagnosticImaging.types.includes(type)}
                        ...
                        <div>
                          ...
                          <Textarea
                            ...
                            value={formData.examinationFindings.diagnosticImaging.notes}
                            onChange={(e) => setFormData(prev => ({
                              ...prev,
                              examinationFindings: {
                                ...prev.examinationFindings,
                            ...
                            placeholder="Why are these investigations needed? Clinical indication and purpose..."
                            disabled={isReadOnly}
                            rows={2}
                          />
                        </div>
                        ...
                        <div>
                          <Label htmlFor="imaging-findings" className="flex items-center">
                            Results & Findings
                          ...
                          <Textarea
                            id="imaging-findings"
                            value={formData.examinationFindings.diagnosticImaging.findings}
                            onChange={(e) => setFormData(prev => ({
                              ...prev,
                              examinationFindings: {
                                ...prev.examinationFindings,
                            ...
                            placeholder="Investigation results and findings (can be updated in follow-up appointments)..."
                            disabled={isReadOnly}
                            rows={4}
                          />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <Label htmlFor="investigation-status">Investigation Status</Label>
                            <Select
                              value={formData.examinationFindings.diagnosticImaging.status || 'ordered'}
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-2 mt-2">
                              <Checkbox
                                ...
                                checked={formData.examinationFindings.diagnosticImaging.followUpRequired || false}
                                onCheckedChange={(checked) => setFormData(prev => ({
                                  ...prev,
                                  examinationFindings: {
                                    ...prev.examinationFindings,
                              ...
                              <Label htmlFor="follow-up-required" className="text-sm">
                                Follow-up appointment needed for results
                              </Label>
                            </div>
                          </div>
                        </div>
            ...
            <TabsContent value="assessment" className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: 'low' | 'moderate' | 'high') => setFormData(prev => ({
                      ...prev,
                      riskAssessment: { ...prev.riskAssessment, cariesRisk: value }
                    }))}
                    disabled={isReadOnly}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: 'low' | 'moderate' | 'high') => setFormData(prev => ({
                      ...prev,
                      riskAssessment: { ...prev.riskAssessment, periodontalRisk: value }
                    }))}
                    disabled={isReadOnly}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: 'routine' | 'semi-urgent' | 'urgent' | 'emergency') =>
                      setFormData(prev => ({ ...prev, treatmentUrgency: value }))
                    }
                    disabled={isReadOnly}
                  >
                    <SelectTrigger>
              ...
              <div>
                <Label htmlFor="examination-notes">Examination Summary Notes</Label>
                <Textarea
                  id="examination-notes"
                  value={formData.examinationNotes}
                  onChange={(e) => setFormData(prev => ({ ...prev, examinationNotes: e.target.value }))}
