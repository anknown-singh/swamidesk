...
import { CalendarIcon, ClockIcon, UserIcon, StethoscopeIcon, AlertTriangleIcon, SearchIcon, CheckIcon, RefreshCwIcon, XCircleIcon, AlertCircleIcon } from 'lucide-react'
...
export function AppointmentBookingForm({
  ...
  const parseError = useCallback((error: unknown, context: string): ApiError => {
    if (!error) {
    ...
    if (typeof error === 'object' && error !== null) {
      ...
      if (supabaseError.code?.startsWith('42')) {
        return {
          ...
          type: 'server',
          code: supabaseError.code,
          details: `Error in ${context}: ${supabaseError.details || supabaseError.message}`
        }
      }

      if (supabaseError.message?.includes('network') || supabaseError.message?.includes('fetch')) {
        return {
  ...
  const setError = useCallback((errorType: keyof ErrorState, error: ApiError) => {
    setErrors(prev => ({
      ...prev,
      [errorType]: error
    }))
  ...
  const retryOperation = useCallback(async (operation: string, retryFunction: () => Promise<void>, maxRetries = 3) => {
    const currentRetryCount = retryCount[operation as keyof typeof retryCount] || 0
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        console.error(`Retry ${currentRetryCount + 1} failed for ${operation}:`, error)
      }
    }, delay)
  }, [retryCount, setError])
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (!formData.scheduled_date) {
      ...
      if (selectedDate < today) {
        newValidationErrors.scheduled_date = 'Cannot schedule appointments in the past'
      }
    }
    ...
    try {
      ...
      try {
        ...
        mappedDoctors = (doctorsData as DoctorData[]).map(doc => ({
          id: doc.id,
          name: doc.full_name,
          department: (doc.user_profiles[0]?.department || 'general').toLowerCase().trim(),
          specialization: doc.user_profiles[0]?.specialization || 'General Practice'
        }))
        ...
        if (mappedDoctors.length === 0) {
          setError('doctors', {
            message: 'No active doctors found. Please ensure doctors are properly configured.',
            type: 'validation'
          })
        }
      } catch (error) {
  ...
  const fetchAvailableTimeSlots = useCallback(async (isRetry = false) => {
  ...
  }, [formData.doctor_id, formData.scheduled_date, parseError, clearError, setError])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (conflictError) {
        throw new Error(`Failed to check time slot availability: ${conflictError.message}`)
      }
  ...
  const ErrorDisplay = ({ error, onRetry, operation }: { 
    error: ApiError | null, 
    onRetry: () => void, 
    operation: string 
  }) => {
    ...
    const getErrorIcon = (type: ApiError['type']) => {
      switch (type) {
        case 'network': return <XCircleIcon className="h-5 w-5 text-red-500" />
        case 'server': return <AlertCircleIcon className="h-5 w-5 text-orange-500" />
        case 'validation': return <AlertTriangleIcon className="h-5 w-5 text-yellow-500" />
        case 'permission': return <XCircleIcon className="h-5 w-5 text-red-500" />
        default: return <AlertCircleIcon className="h-5 w-5 text-gray-500" />
      }
    }
    ...
    const getErrorColor = (type: ApiError['type']) => {
      switch (type) {
        ...
        case 'server': return 'border-orange-200 bg-orange-50'
        case 'validation': return 'border-yellow-200 bg-yellow-50'
        case 'permission': return 'border-red-200 bg-red-50'
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 text-left">
                            <div className="font-medium text-sm leading-tight">{patient.full_name}</div>
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className="h-auto min-h-[2.5rem] py-2">
                  ...
                  {console.log('Current department in form:', formData.department)}
                </SelectTrigger>
                <SelectContent>
                  {departments.map((dept) => (
                    <SelectItem key={dept.value} value={dept.value} className="py-2 text-left">
                      {dept.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
                ...
                <SelectTrigger className="h-auto min-h-[3rem] py-3">
                  <SelectValue placeholder={
                    formData.department && filteredDoctors.length === 0
                      ? `No doctors available in ${departments.find(d => d.value === formData.department)?.label || 'selected department'}`
                      : "Select doctor"
                  } />
                </SelectTrigger>
                <SelectContent>
                  {filteredDoctors.length > 0 ? (
                    filteredDoctors.map((doctor) => (
                      <SelectItem key={doctor.id} value={doctor.id} className="py-3">
                        <div className="text-left w-full">
                          <div className="font-medium leading-tight">{doctor.name}</div>
                    ...
                    <div className="px-4 py-6 text-center text-sm text-muted-foreground">
                      <div className="mb-2">No doctors available in {departments.find(d => d.value === formData.department)?.label}</div>
                      <div className="text-xs">Try selecting a different department</div>
                    </div>
                  ) : (
                    <div className="px-4 py-6 text-center text-sm text-muted-foreground">
              ...
              {formData.department && filteredDoctors.length === 0 && (
                <div className="text-sm text-orange-600 bg-orange-50 border border-orange-200 rounded-md p-3">
                  ...
                  <p className="text-xs">No doctors are currently assigned to the {departments.find(d => d.value === formData.department)?.label} department. Please select a different department or contact administration to add doctors to this department.</p>
                </div>
              )}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="15" className="py-2 text-left">15 minutes</SelectItem>
                  <SelectItem value="30" className="py-2 text-left">30 minutes</SelectItem>
                  <SelectItem value="45" className="py-2 text-left">45 minutes</SelectItem>
                  <SelectItem value="60" className="py-2 text-left">1 hour</SelectItem>
                  <SelectItem value="90" className="py-2 text-left">1.5 hours</SelectItem>
                  <SelectItem value="120" className="py-2 text-left">2 hours</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-3">
              <Label htmlFor="scheduled_time" className="flex items-center gap-2 text-base font-medium">
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              Include patient symptoms, reason for visit, or any special instructions
            </div>
          </div>
