...
-- =====================================================
-- Comprehensive clinic management system schema
-- Creates all 29 tables with relationships, functions, and triggers
-- Based on database-reset/create-all-tables.sql
...
-- =====================================================
-- STEP 3: Core System Tables (9 tables)
-- =====================================================
...
-- =====================================================
-- STEP 4: Medicine & Pharmacy Tables (8 tables)
-- =====================================================
...
-- =====================================================
-- STEP 5: Appointment System Tables (6 tables)
-- =====================================================
...
-- =====================================================
-- STEP 6: Clinical Management Tables (3 tables)
-- =====================================================
...
-- =====================================================
-- STEP 7: Supporting Tables (3 tables)
-- =====================================================
...
-- Medicine master update trigger function
...
CREATE OR REPLACE FUNCTION medicine_search_text(med_name text, gen_name text, brand_arr text[])
RETURNS TEXT AS $$
BEGIN
    RETURN LOWER(COALESCE(med_name, '') || ' ' || 
                 COALESCE(gen_name, '') || ' ' || 
                 COALESCE(array_to_string(brand_arr, ' '), ''));
END;
$$ LANGUAGE plpgsql;
...
CREATE INDEX idx_appointments_date_doctor ON appointments(scheduled_date, doctor_id);
...
CREATE INDEX idx_sell_orders_date ON sell_orders(sale_date);
CREATE INDEX idx_sell_order_items_order ON sell_order_items(sell_order_id);

-- =====================================================
-- STEP 10: Create Triggers
-- =====================================================

-- Set appointment number trigger
...
CREATE TRIGGER update_medicine_master_updated_at_trigger 
    BEFORE UPDATE ON medicine_master 
    FOR EACH ROW EXECUTE FUNCTION update_medicine_master_updated_at();

CREATE TRIGGER update_pharmacy_issues_updated_at 
...
CREATE TRIGGER update_visit_services_updated_at 
...
-- =====================================================
-- STEP 11: Queue Management (Real-time view)
-- =====================================================
...
SELECT 
    ...
    p.phone as patient_mobile,
    u.full_name as doctor_name,
    ROW_NUMBER() OVER (ORDER BY v.visit_time ASC) as queue_position
FROM visits v
